...
class Benchmark():
    ...
    def run_simple_benchmark(self, i: int, stage: int,
    ...
    def run_combined_benchmark(self, i: int, stage: int,
                               total_stages: int) -> int:
        ...
        if VERBOSE:
            ...
            utils.printProgressBar(stage,
                                   ...
                                   show_eta=True)
        start = time.time()
        _, v = self.algo.run_sa(dir=f"{CWD}/benchmark", name="SA", path=p)
        end = time.time()
        if v < lb:
            raise ValueError("Lower bound broken")
        self.results["NNSA"].append(
            [v, (end - start), abs(100 - ((100 * v) / lb))])
        ...
        if VERBOSE:
            ...
            utils.printProgressBar(stage,
                                   ...
                                   show_eta=True)
        start = time.time()
        _, v = self.algo.run_tabu_search(dir=f"{CWD}/benchmark",
                                         name="TS",
                                         path=p)
        end = time.time()
        if v < lb:
    ...
    def generate_results(self):
        ...
        if MODE == 0:
            ...
            for i, l in enumerate(values):
                ...
                at, apt, ap = (sum(t[0]) / len(t[0]), sum(t[1]) / len(t[1]),
                               sum(t[2]) / len(t[2]))
                mit, mipt, mip = min(t[0]), min(t[1]), min(t[2])
                table.append([
                    keys[i], f"{mit:.2f}", f"{at:.2f}", f"{mt:.2f}",
                    f"{mipt:.2f}", f"{apt:.2f}", f"{mpt:.2f}", f"{mip:.2f}",
                    f"{ap:.2f}", f"{mp:.2f}"
                ])
